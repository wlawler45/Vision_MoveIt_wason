...
def main():
    ...
    board_panel=cv2.aruco.GridBoard_create(8, 3, 0.025, 0.0075, aruco_dict, 50)
...
#    boards=[gripper_board, panel_board, ref1, ref2, ref3, ref4]
    ...
    loaded_object_points_ground_in_panel_system_stage_1 = loadmat('/home/rpi-cats/Desktop/DJ/Ideal Position/Cam_636_object_points_ground_tag_in_panel_frame_Above_Nest.mat')['object_points_ground_in_panel_tag_system']
...
#    while ((du>10) | (dv>10) | (pose_target2.p[2] > 0.172)):    #try changing du and dv to lower values(number of iterations increases)
    while ((du>1) | (dv>1) and (iteration<125) ):    #try changing du and dv to lower values(number of iterations increases)
#    while ((np.max(np.abs(dutmp))>0.5) | (np.max(np.abs(dvtmp))>0.5)):    #try changing du and dv to lower values(number of iterations increases)
        ...
        if((du<2) and (dv<2) and (stage==1)):
			...
			filename_pose1 = "/home/rpi-cats/Desktop/DJ/Code/Data/Above_Nest_Pose_"+str(t1)+".mat"
...
#        print "combined_list:",combined_list
        ...
        Z = 1*P[20:40,2] #- [0.68184539, 0.68560932, 0.68966803, 0.69619578])
...
#            cv2.waitKey(1)
            ...
            for ic in range(4):
                ...
                reprojected_imagePoints_ground_2 = np.reshape(reprojected_imagePoints_ground_2,(20,8))
                UV_target = np.vstack([reprojected_imagePoints_ground_2[:,2*ic]-1209.151959040735,reprojected_imagePoints_ground_2[:,2*ic+1]-1055.254852652610]).T
                uc = UV_target[:,0]
                vc = UV_target[:,1]
    		    
#                print 'UV_target', UV_target
                UV_current = np.vstack([UV[20:40,2*ic]-1209.151959040735,UV[20:40,2*ic+1]-1055.254852652610]).T
...
#                print 'delta_UV: ',ic, delta_UV
                ...
                if ic ==0:
                    ...
                    delta_UV_all = np.vstack([delta_UV_all,delta_UV.reshape(40,1)]) 
                    UV_target_all = np.vstack([UV_target_all,UV_target.reshape(40,1)])
...
#            print 'dx:',dx             #translational and angular velocity
            ...
            goal = trapezoid_gen(np.array(current_joint_angles) + joints_vel.dot(dt),np.array(current_joint_angles),t_duration)
...
#    controller_commander.compute_cartesian_path_and_move(pose_target2, avoid_collisions=False)
    ...
    #Load ideal pose differnece information from file
    loaded_rvec_difference = loadmat('/home/rpi-cats/Desktop/DJ/Ideal Position/Cam_636_object_points_ground_tag_in_panel_frame_In_Nest.mat')['rvec_difference']
    loaded_tvec_difference = loadmat('/home/rpi-cats/Desktop/DJ/Ideal Position/Cam_636_object_points_ground_tag_in_panel_frame_In_Nest.mat')['tvec_difference']
